...
def init(run_enter = None, St = None, Cr = None, Pe = None, Da_h = None, 
    ...
    xx, zz_p = np.meshgrid(x_f, z_c, indexing = 'ij')   # horizontal faces
    ...
    T_Bice[xin[:, 0] < a_0[Nz_b]] = \
        -T_0/a_0[Nz_b]**2*xin[xin[:, 0] < a_0[Nz_b], 0]**2 + T_0
    T_Bice[xin[:, 0] >= a_0[Nz_b]] = \
        c_Br[0]*(xin[xin[:, 0] >= a_0[Nz_b], 0]**3 - a_0[Nz_b]**3) + \
        c_Br[1]*(xin[xin[:, 0] >= a_0[Nz_b],  0]**2 - a_0[Nz_b]**2) + \
        c_Br[2]*(xin[xin[:, 0] >= a_0[Nz_b], 0] - a_0[Nz_b])
    ...
    T_Tice[xin[:, -1] < a_0[Nz_t-1]] = \
        T_0/a_0[Nz_t-1]**2*(2/a_0[Nz_t-1]*xin[xin[:, -1] < a_0[Nz_t-1], -1]**3 - \
                        3*xin[xin[:, -1] < a_0[Nz_t-1], -1]**2) + T_0
    T_Tice[xin[:, -1] >= a_0[Nz_t-1]] = CC_0[xin[:, -1] >= a_0[Nz_t-1], Nz_t]
    ...
    for i in range(Nx):
        if T_Tice[i] > 0:
    #        TT_0[i, Nz_b:Nz_t] = AR*(T_Tice[i] - T_Bice[i])*zin[i, Nz_b:Nz_t] + T_Bice[i]
    ...
    # derivatives at top/bottom of ice (dimensionless)
    ...
    # left/right boundary temperatures in pond/ocean
    # temperature in pond
    ...
    if BC_T == None and BC_p == None:
        BC_T = int(input('Enter BC types for T, H, C, phi (L, R, B, T) - '+\
                      '0 = (N, N, D, D), 1 = (N, N, N, D), 2 = (D, D, N, D) (default 0) : ') or 0)
