...
# procedure. Used with init_2D_t for channel with tributaries.
#========================================================================
...
if cont == 1:
    ...
    Pe = float(input('Enter Peclet number (default = {}) : '.format(100)) or 100)
    ...
    a_0_t = float(input('Enter tributary radii (default 0.1*AR) : ') or 0.1*AR)
    p_an = float(input('Enter tributary angle (default {}) : '.format(4*np.pi/32)) or 4*np.pi/32)
    z_pos = float(input('Enter vertical position of tributaries (default {}) : '.format(0.5)) or 0.5)
    a_len = float(input('Enter length of tributaries (default {}) : '.format(0.2)) or 0.2)
...
# tributary position
i_pos = np.array([0.15, z_pos])

if cont == 0:
    ic.init(run_enter = run_name, St = St, Cr = Cr, Pe = Pe, Da_h = Da_h, 
             T_p = T_p, C_p = C_p, AR = AR, Nz = Nz, j = a_0_int, a_0_t = a_0_t, 
             a_len = a_len, p_an = p_an, i_pos = i_pos,
