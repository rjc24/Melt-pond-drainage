...
# example initial conditions (rotated channel)

def init(run_enter = None, St = None, Cr = None, Pe = None, Da_h = None, 
         T_p = None, C_p = None, AR = None, Nz = None, j = None, 
    ...
    a_centre[:] = i_pos[0] + (z_c[:] - i_pos[1])*np.tan(p_an)
    np.putmask(a_0n[0], np.logical_and(z_c[:] >= ice_b, z_c[:] <= ice_t), 
               a_centre[:] - a_0[:]/(np.cos(p_an)))
    ...
    a_centre_top = i_pos[0] + (ice_t - i_pos[1])*np.tan(p_an)
    a_l_t = a_centre_top - a_0[Nz_t-1]/(np.cos(p_an))
    a_r_t = a_centre_top + a_0[Nz_t-1]/(np.cos(p_an))
    
    np.putmask(C_ice_t, np.logical_and(xin[:, Nz_t] < a_r_t, xin[:, Nz_t] > a_l_t), 
                -0.3)
