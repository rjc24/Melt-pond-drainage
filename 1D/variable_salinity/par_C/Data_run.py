...
C_p = float(input('Enter pond salinity (default -0.3) : ') or -0.3)
Re = float(input('Enter Reynolds number (default 25) : ') or 25)
...
ic.init(run_enter = run_name, St = St, Cr = Cr, Re = Re, 
        T_p = T_p, C_p = C_p, lam = lam, j = j, Nx = Nx, 
        t_run = t_run, BC = BC)
...
a_t_file = open('arrays/a_t.asc', 'r')
dt_file = open('arrays/dt_max.asc', 'r')
St_file = open('arrays/St.asc', 'r')
Cr_file = open('arrays/Cr.asc', 'r')
Re_file = open('arrays/Re.asc', 'r')
Pr_file = open('arrays/Pr.asc', 'r')
T_p_file = open('arrays/T_p.asc', 'r')
C_p_file = open('arrays/C_p.asc', 'r')
lam_file = open('arrays/lam.asc', 'r')
...
a_t = float(np.loadtxt(a_t_file))
St = float(np.loadtxt(St_file))
Cr = float(np.loadtxt(Cr_file))
Re = float(np.loadtxt(Re_file))
Pr = float(np.loadtxt(Pr_file))
T_p = float(np.loadtxt(T_p_file))
C_p = float(np.loadtxt(C_p_file))
lam = float(np.loadtxt(lam_file))
...
p_vals = [T_p, C_p]   # pond temp/salinity
...
HH_all, TT_all, CC_all, a_all, tt_all = \
    data_call(run, 'Crank', HH_0, TT_0, CC_0, xx, xin, tt_0, tt_shots, 
              ...
              omega_s = 1, omega_i = 1, 
              tol_a = 1e-8, tol_r = 1e-8, tol_g = 1e-6, max_iter = 250,
              shots = shots)
