...
def A_theta(method, xx, xin, dt, cc_p, k_b, alp, D_n, EQN = 1):
...
#================================= Main body =================================
    ...
    main[1:-1] = (xx[2:-1]**2 - xx[1:-2]**2)/2 + \
        ...
        k_b[1:-2]*xx[1:-2]/(xin[1:-1] - xin[:-2]))/cc_p[1:-1] + \
        (D_n*xx[2:-1]/(xin[2:] - xin[1:-1]) + \
        D_n*xx[1:-2]/(xin[1:-1] - xin[:-2])))*dt*theta

    lower[:-1] = -(k_b[1:-2]/cc_p[:-2] + D_n)*xx[1:-2]/(xin[1:-1] - xin[:-2])*dt*theta
    upper[1:] = -(k_b[2:-1]/cc_p[2:] + D_n)*xx[2:-1]/(xin[2:] - xin[1:-1])*dt*theta
...
#=============================== Left boundary ===============================
    ...
    main[0] = (xx[1]**2 - xx[0]**2)/2 + \
        ...
        9*k_b[0]*xx[0]*alp_L/(6*(xin[0] - xx[0])))/cc_p[0] + \
        (D_n*xx[1]/(xin[1] - xin[0]) + \
        9*D_n*xx[0]*alp_L/(6*(xin[0] - xx[0]))))*dt*theta
                        
    upper[0] = -((k_b[1]*xx[1]/(xin[1] - xin[0]) + \
        k_b[0]*xx[0]*alp_L/(6*(xin[0] - xx[0])))/cc_p[1] + \
        (D_n*xx[1]/(xin[1] - xin[0]) + \
        D_n*xx[0]*alp_L/(6*(xin[0] - xx[0]))))*dt*theta
...
#=============================== Right boundary ===============================
    ...
    main[-1] = (xx[-1]**2 - xx[-2]**2)/2 + \
        ...
        k_b[-2]*xx[-2]/(xin[-1] - xin[-2]))/cc_p[-1] + \
        (9*D_n*xx[-1]*alp_R/(6*(xx[-1] - xin[-1])) + \
        D_n*xx[-2]/(xin[-1] - xin[-2])))*dt*theta
        
    lower[-1] = -((k_b[-2]*xx[-2]/(xin[-1] - xin[-2]) + \
        k_b[-1]*xx[-1]*alp_R/(6*(xx[-1] - xin[-1])))/cc_p[-2] + \
        (D_n*xx[-2]/(xin[-1] - xin[-2]) + \
        D_n*xx[-1]*alp_R/(6*(xx[-1] - xin[-1]))))*dt*theta
