...
from Enthalpy_functions import SFrac_calc, Q_Vflux, Diff_bounds, \
    Temp_calc, LConc_scal, SFrac_bounds
from int_basic import int_quad


def NL_solve(method, HH_0, HH_p, CC_0, CC_p, xx, xin, t1, t2, 
             a_0, a, Q_T, params, BC_D, BC_N, alp, G_T = None, G_C = None, 
    ...
    n = 0  # Counter
    HH, HH_del, CC, CC_del, TT, phi_0, phi, chi_p0, chi_p, cc_p0, cc_p = \
        map(np.zeros, 11*(np.size(HH_0),))
    phi_0[:], phi[:] = SFrac_calc(HH_0, CC_0, St, Cr), SFrac_calc(HH_p, CC_p, St, Cr)
    TT[:] = Temp_calc(HH_p, CC_p, phi, St, Cr)
    CC[:] = CC_p
