...
def b_theta(method, HH_0, HH, phi_0, phi, xx, xin, dt, 
            ...
            FF_0, FF, FF_phi_0, FF_phi, St, Cr, alp, G = None, EQN = 1):
    ...
    #    k_b0, k_b - thermal conductivity / solutal diffusivity
    #                               at times n, n+1
    ...
    # Initializing b
    b, time_p, advec, diffus, diffus_p, s_frac, s_frac_p, source_n, source_a = \
        map(np.zeros, 9*(Nx, ))
...
#=============================================================================
    ...
    b[:] = time_p[:] + diffus[:] + diffus_p[:] + \
        s_frac[:] + s_frac_p[:] + source_n[:] + source_a[:] - advec[:]
