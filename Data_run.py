...
if cont == 1:
    ...
    Pe = float(input('Enter Peclet number (default = {}) : '.format(ppar.Pe_m)) or ppar.Pe_m)
    Da_h = float(input('Enter Hele-Shaw Darcy number (default = {0:.2f}) : '.format(ppar.Da_h)) or ppar.Da_h)
    T_p = float(input('Enter pond temperature (default = {}) : '.format(0.8)) or 0.8)
    ...
    j_int = int(input('Enter interface index : '))
...
if cont == 0:
    ic.init(run_enter = run_name, St = St, Cr = Cr, Pe = Pe, 
            Da_h = Da_h, T_p = T_p, C_p = C_p, 
            AR = AR, Nz = Nz, j = j_int, BC_T = BC_T, 
            BC_p = BC_p, t_run = t_run)
else:
